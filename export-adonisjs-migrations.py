...
# A MySQL Workbench plugin which exports all Model in the existing database to Knex schema in AdonisJS migrations template
...
ModuleInfo = DefineModule(name='GenerateAdonisJSMigration', author='Brandon Eckenrode & Mahendaran', version='1.0.0')
migrations = {}
migration_tables = []
...
@ModuleInfo.plugin('wb.util.generate_adonisjs_migration',
                   caption='Export AdonisJS Migration',
                   input=[wbinputs.currentCatalog()],
                   groups=['Catalog/Utilities', 'Menu/Catalog']
                   )
@ModuleInfo.export(grt.INT, grt.classes.db_Catalog)
def generate_adonisjs_migration(cat):
    ...
    except GenerateAdonisJSMigrationError as e:
    ...
    wizard = GenerateAdonisJSMigrationWizard(sql_text)
    wizard.run()

    return 0


class GenerateAdonisJSMigrationError(Exception):
...
class GenerateAdonisJSMigrationWizardPreviewPage(WizardPage):
...
class GenerateAdonisJSMigrationWizard(WizardForm):
    def __init__(self, sql_text):
        ...
        self.preview_page = GenerateAdonisJSMigrationWizardPreviewPage(self, sql_text)
